 ...
 * 👫 (2026-09-20 مساءً «تاريخ الزوجة لحال وتاريخ الأولاد لحال»): مدّتان مستقلّتان — family_allowance_spouse_from/to
 * وfamily_allowance_children_from/to؛ العمودان القديمان (family_allowance_from/to) يُنسخان مرّة إلى المدّتين ثم يبقيان بلا استعمال.
 */
function ensureFamilyAllowanceDateColumns(): void {
    ...
    try {
        ...
        if ($fresh) {
            $db->exec("UPDATE employees SET family_allowance_spouse_from = family_allowance_from, family_allowance_spouse_to = family_allowance_to,
...
/** هل الشهر ضمن مدّة تعويض الزوجة ('spouse') أو الأولاد ('children') بملفه؟ (from فارغ = من الأزل، to فارغ = مستمرّ؛ to شامل لشهره) */
function familyAllowanceMonthInWindow(array $emp, int $month, int $year, string $kind = 'children'): bool {
    $k = $year * 12 + $month;
    $kind = $kind === 'spouse' ? 'spouse' : 'children';
    $f = familyAllowanceDateKey($emp["family_allowance_{$kind}_from"] ?? '');
    $t = familyAllowanceDateKey($emp["family_allowance_{$kind}_to"] ?? '');
 ...
 */
/** 👩 «بعد بدك تضيف الآنسة على الجنس» (2026-09-15): القيمة الثالثة بخانة الجنس = 'd' (Mademoiselle / الآنسة).
 *  صيغتها مؤنّثة بكل النصوص، والفرق باللقب فقط؛ ونماذج الدولة (× ذكر/أنثى) تعاملها أنثى — هذه الدالة المصدر الواحد. */
function genderSexOf($g): string {
    $g = (string)$g;
    if ($g === 'd') return 'f';
    return in_array($g, ['m', 'f'], true) ? $g : '';
}
...
// School Year
// =====================================================
// السنة الدراسية بحسب التقويم وحده: تبدأ في تشرين الأول (10) وتنتهي في أيلول (9).
function calendarSchoolYear() {
    $month = (int)date('n');
    $year = (int)date('Y');
    if ($month >= 10) {
        return $year . '-' . ($year + 1);
...
/**
 * 📅 السنة الحالية للبرنامج (2026-09-11 «السنة الجديدة بس نفتحها لازم تلقائياً تطبّق على كل التقارير والإفادات وكل شي»):
 * بمجرّد فتح السنة الجديدة (صفحة فتح سنة) تُسجَّل بالإعداد program_school_year فيصير البرنامج كله عليها
 * (منتقي السنة الافتراضي، التقارير، الإفادات، القسائم، لوحة القيادة…) بلا انتظار 1 تشرين. لا تسمح بالرجوع
 * لسنة أقدم من التقويم (الإعداد يُعتمَد فقط إن كان ≥ سنة التقويم). حذف/تفريغ السنة يُرجِع الافتراضي.
 */
function currentSchoolYear() {
    $cal = calendarSchoolYear();
    $forced = (string)getSetting('program_school_year', '');
    if (preg_match('/^\d{4}-\d{4}$/', $forced) && strcmp($forced, $cal) >= 0) return $forced;
    return $cal;
}
...
function healFamilyAllowanceFrom20260920() {
    try {
        ...
        require_once __DIR__ . '/payroll_calculator.php';
        $emps = $db->query("SELECT e.id, CONCAT(COALESCE(NULLIF(e.first_name_ar,''),e.first_name_fr),' ',COALESCE(NULLIF(e.last_name_ar,''),e.last_name_fr)) nm
            FROM employees e WHERE e.is_deleted = 0 AND e.family_allowance_spouse_from IS NULL AND e.family_allowance_children_from IS NULL
        ...
        foreach ($emps as $e) {
            ...
            $db->prepare("UPDATE employees SET family_allowance_from = ?, family_allowance_spouse_from = ?, family_allowance_children_from = ? WHERE id = ?")->execute([$from, $from, $from, $eid]);
            $n = 0;
 ...
 * مرّة واحدة (فلاغ heal_abra_pct65v2_20260910 done: …) — بلا زرّ، يشتغل بأول فتح صفحة أونلاين.
 */
function healAbraPct65_20260910() {
    try {
        if (strpos((string)getSetting('heal_abra_pct65v2_20260910', ''), 'done') === 0) return;
        $db = getDB();
        require_once __DIR__ . '/payroll_calculator.php';
        $sid = (int)$db->query("SELECT id FROM schools WHERE name_ar LIKE '%ثانوية السيدة%' ORDER BY id LIMIT 1")->fetchColumn();
        if ($sid <= 0) { setSetting('heal_abra_pct65v2_20260910', 'done: no school'); return; }
        ...
        $targets = $db->query("SELECT b.id bid, b.employee_id eid, b.value_type, b.amount,
            ...
            WHERE e.school_id=$sid AND e.employee_type='enseignant_titulaire'
              ...
              -- v2 (أمره 2026-09-10 «دايما طبّق القانون بكل البرنامج»): ريتا مارون وماريا الياس حليحل تُحوَّلان أيضاً (مبلغاهما = 65٪)
              -- وتُعاد أشهرهما بالقانون (كانت مجمّدة على الكشف القديم رغم درجات 4+4+2 بكانون 2026) — لا محميّين بعد اليوم.
            ORDER BY b.employee_id, b.id")->fetchAll(PDO::FETCH_ASSOC);
        $log = []; $done = [];
        $prev = (string)getSetting('heal_abra_pct65v2_log', '');
        $n = 0;
        foreach ($targets as $t) {
            $eid = (int)$t['eid'];
            if ($n >= 6 && !isset($done[$eid])) { setSetting('heal_abra_pct65v2_log', $prev . implode('؛ ', $log) . '؛ '); setSetting('heal_abra_pct65v2_progress', 'working... ' . count($done)); return; }
        ...
        setSetting('heal_abra_pct65v2_log', $prev . implode('؛ ', $log));
        $all = (string)getSetting('heal_abra_pct65v2_log', '');
        setSetting('heal_abra_pct65v2_20260910', 'done: converted=' . substr_count($all, '→65٪') . ' | ' . mb_substr($all, 0, 1500));
        setSetting('heal_abra_pct65v2_progress', 'done');
    } catch (Throwable $e) {
        try { setSetting('heal_abra_pct65v2_progress', 'err: ' . mb_substr($e->getMessage(), 0, 200)); } catch (Throwable $e2) {}
    }
}
