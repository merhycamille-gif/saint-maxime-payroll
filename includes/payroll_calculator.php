...
/**
 * 🆕 أستاذ «نظام الأساتذة الجدد (4+4+2)» = دخل الملاك **بعد 2/4/2012** (تاريخ قانون 223).
 * المصدر الواحد لهذا الحدّ (يستعمله بناء الدرجات، وعدد درجات كل قانون، ولوائح الإعطاء):
 * هذا الأستاذ يأخذ 4+4+2 = 10 درجات **بديلاً** عن القوانين التاريخية 244/102/223 فلا تنطبق عليه.
 * (جوزف السرّوع 2026-09-10: «p1 بتقول عاطيهن وp2 مش عاطيهن» — كانت 244/102/223 تظهر «بعدها ما أُعطيت».)
 */
function isNewSystemTeacher($emp) {
    $d = employeeEntryDate($emp);
    return $d ? (strtotime($d) > strtotime('2012-04-02')) : false;
}
...
function buildLegalGradeHistory($empId, $todayOverride = null, $dryRun = false, $force = false) {
    ...
    $isNew = isNewSystemTeacher($emp);              // أستاذ جديد (نظام 4+4+2) — المصدر الواحد
    ...
    if (!$isNew) {
        ...
        // «يُقدَّم تدرّجهم سنة واحدة» = نصف درجة إضافية تظهر عند **أوّل تشرين بعد دخول الملاك (mAY+1)** — لا عند mAY+3.
        // 🔴 صحّحها كشف ملاك عبرا تشرين 2026 (2026-09-12، «لازم يطلعوا هني ذاتهن تبع السنة»): ايليو نوفل/ريتا الحايك… (ملاك 10/2024)
        //    عنده 20 بتشرين 2025 لا 19.5 و24 بكانون 2026؛ اندي يونان (ملاك 10/2025) 20 بتشرين 2026 لا 19.5؛ ماريا حليحل (إجازة
        //    جامعية، ملاك 10/2024) 16 بكانون 2026 لا 15.5 — أي أنّ نصف «تقديم التدرّج» يُضاف مع أوّل نصف عادي فيكتمل درجةً كاملةً.
        //    (كان mAY+3 فحرم دفعتَي 2023 و2024 من نصف درجة سنتين — «الـ13 درجة دفعة 2023» المعلّقة). تُضاف بعد حلول تاريخها فقط.
        $compYear = $mAY + 1;
        if (strtotime($compYear . '-10-01') <= $todayTs) {
            $events[] = ['date' => $compYear . '-10-01', 'type' => 'ordinary', 'delta' => 0.5, 'comp' => true];
        }
...
function lawGradesForEmployee($law, $emp) {
    $ln = (string)$law['law_number'];
    if ($ln === '2017') return law2017GradesFor($emp);
    // القوانين التاريخية 244/102/223 لا تنطبق على أستاذ نظام 4+4+2 (بديلها) — نفس حدّ buildLegalGradeHistory.
    if (in_array($ln, ['244', '102', '223'], true) && isNewSystemTeacher($emp)) return 0.0;
