...
function complianceItems(PDO $db, string $sy): array {
    ...
    foreach ($q("SELECT e.* FROM employees e
        ...
        $hasCfg = salaryEngineAllowed($r, $db) && (($r['employee_type'] === 'enseignant_titulaire') || (float)$r['base_salary_usd'] > 0 || (float)$r['contract_salary_lbp'] > 0 || isLaborLawSalary($r) || salaryYearPayable((int)$r['id'], $sy, $db));
