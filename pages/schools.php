...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $data = [
        ...
        $addrAr,
        $addrFr,
        trim(arabicDigitsFr($_POST['phone'] ?? '')), // «كل الارقام اكتبو بالفرنسي»
        trim($_POST['email'] ?? ''),
        officialNumberFr($_POST['nssf_employer_number'] ?? ''),
        officialNumberFr($_POST['finance_number'] ?? ''),
        officialNumberFr($_POST['caisse_number'] ?? ''),
        $dirAr,
        $dirFr,
