...
<?php if (!$report):
    ...
    $categories = [
        ...
        ['title'=>'الضمان الاجتماعي / CNSS', 'icon'=>'fa-hospital', 'color'=>'var(--info)', 'tiles'=>[
            ...
            ['url'=>$OF.'cnss_eos_doc', 'icon'=>'fa-file-signature', 'color'=>'var(--danger)', 'fr'=>'Liquidation FDS — document (p1)', 'ar'=>'مستند تصفية تعويض نهاية خدمة'],
            ['url'=>$OF.'cnss_eos_2y', 'icon'=>'fa-table-list', 'color'=>'var(--danger)', 'fr'=>'FDS — salaires 2 dernières années (p2)', 'ar'=>'نهاية الخدمة — جدول أجور السنتين الأخيرتين'],
            ['url'=>$OF.'cnss_eos_annual', 'icon'=>'fa-table', 'color'=>'var(--danger)', 'fr'=>'FDS — salaires annuels (p3)', 'ar'=>'نهاية الخدمة — جدول بالأجور السنوية'],
                    ...
                    <tbody>
                        ...
                        $sumRow = function($label, $t, $isGrand) use ($colspanLbl, $dualTot) {
                            ...
                            <tr class="<?= $isGrand ? 'total-row' : 'subtotal-row' ?>" style="<?= $bg ?>font-weight:700">
                                ...
                                <td><?= $dualTot($t['net'], $t['net_usd']) ?></td><td><?= $dualTot($t['family'], $t['family_usd']) ?></td><?= netFamTd($dualTot($t['net'] + $t['family'], $t['net_usd'] + $t['family_usd']), '') ?><?= transportTd($dualTot($t['trans'], $t['trans_usd']), '') ?>
                                <?= dueTd('<strong>' . $dualTot($t['total'], $t['total_usd']) . '</strong>', '') ?>
                            </tr>
                        ...
                        foreach ($data as $r):
                            ...
                            $catN++;
                            echo categoryHeaderRow($curCat, $cat, ($multi?18:17) + compColsCount() + netFamColsCount());
                        ?>
                            <tr>
                                <td><?= ++$rn ?></td>
                                ...
                                <?= netFamTd(money(netFamLbp($r), $rRate), '') ?>
                                <?= transportTd(money($rTrans, $rRate), '') ?>
                                <?= dueTd('<strong>' . money(dueShownLbp($r), $rRate) . '</strong>', '') ?>
                            </tr>
                        <?php endforeach; ?>
                    ...
                    <tbody>
                        ...
                        $drawTotal = function($label,$a,$isGrand) use ($csL, $repRate){
                            ...
                            <tr class="<?= $cls ?>" style="<?= $bg ?>font-weight:700"><td colspan="<?= $csL ?>" style="text-align:right"><?= e($label) ?></td>
                                <td><?= moneyLaw($a['base']) ?></td><?php if (salaryCompHas('extra')): ?><td><?= dualFromUsd($a['extra'], $a['extra_usd']) ?></td><?php endif; ?><?php if (salaryCompHas('aide')): ?><td><?= money($a['aide'], $repRate) ?></td><?php endif; ?>
                                <td style="background:#eef2ff"><strong><?= dualFromUsd($a['composed'], $a['composed_usd']) ?></strong></td>
                        ...
                        foreach ($data as $r):
                            ...
                            $add=['base'=>(int)$r['base_salary_lbp'],'extra'=>extraWageLbp($r),'extra_usd'=>extraWageUsd($r),'aide'=>aideCompLbp($r),'composed'=>composedSalaryLbp($r),'composed_usd'=>composedSalaryUsd($r),'cnss'=>(int)$r['cnss_amount_lbp'],'school'=>(int)$r['school_cnss_8_lbp']];
        ...
        <?= docSheetStart('Caisse EOC — retenues mensuelles', 'كشف صندوق التعليم الخاص الشهري', [monthName($month) . ' ' . $year], ['month' => $month, 'year' => $year]) ?>
                <div class="report-table-wrap" dir="rtl"><table class="doc-table" dir="rtl">
                    ...
                    <tbody>
                        ...
                        <tr class="total-row"><td colspan="<?= $multi?4:3 ?>">المجاميع — العدد: <?= $rn ?></td><td><?= moneyLaw($teBaseE) ?></td><?php if (salaryCompHas('extra')): ?><td><?= dualFromUsd($teEx, $teExU) ?></td><?php endif; ?><?php if (salaryCompHas('aide')): ?><td><?= money($teAi, $repRate) ?></td><?php endif; ?><td style="background:#eef2ff"><strong><?= dualFromUsd($teComposed, $teComposedU) ?></strong></td><td><?= money($te, $repRate) ?></td><td><?= money($teg, $repRate) ?></td><td><?= money($ts, $repRate) ?></td></tr>
                        <?php endif; ?>
                    </tbody>
                </table></div>
        <?= docSheetEnd() ?>
        ...
        <?= docSheetStart($tLblFr, $tLblAr, ['العدد: ' . count($data)], ['comp' => false]) ?>
                <div class="report-table-wrap" dir="rtl"><table class="doc-table" dir="rtl">
                    <thead><tr>
                        ...
                        <th>الاسم / Nom</th><th>الشهادة / Diplôme</th><th>الدرجة الحالية / Échelon</th>
                        <th>دخول المدرسة / Embauche</th><th>دخول الملاك / Titularisation</th><th>الحالة / Statut</th>
                    </tr></thead>
                    <tbody>
                        <?php
                        $colsN = ($multi ? 8 : 7);
