...
function ofAnnualAgg($db, $empId, $schoolYear) {
    $stmt = $db->prepare("SELECT
        ...
        SUM(" . extraWageUsdSql('') . ") AS extra_wage_usd,
    ...
    <div class="card no-print" style="max-width:860px;margin:0 auto">
        ...
        <div class="card-body">
            ...
            <div style="display:flex;gap:12px;flex-wrap:wrap">
                ...
                <a class="btn btn-lg" style="background:#0e7490;color:#fff" href="<?= e(BASE_URL . 'pages/official_export.php?form=mof_r567&fy=' . $fy . $fltQ) ?>&check=1" target="_blank"><i class="fas fa-magnifying-glass"></i> تدقيق أسماء المناطق (أكواد الوزارة)</a>
                <a class="btn btn-lg" style="background:#166534;color:#fff" href="<?= e(BASE_URL . 'pages/r567_check.php') ?>"><i class="fas fa-file-shield"></i> 🔍 فحص ملف XML قبل الإرسال / Vérifier le XML</a>
            </div>
            <p class="text-muted mt-3"><i class="fas fa-info-circle"></i> زرّ <strong>«ملف الوزارة R567»</strong> ينزّل ملف الوزارة الرسمي بالماكرو (R5 + R6 صف لكل موظف + R7 التاركون) معبّأً بأرقام السنة — افتحه بإكسل واكبس زرّ التوليد فيه فيطلع ملف R567.xml للرفع على بوابة المالية. النموذج يطلع <strong>طبق الأصل عن نموذج ر5 الرسمي</strong> (قالب ملفك نفسه) معبّأً تلقائياً: المؤسسة ورقمها المالي وعنوانها والمكلف بتبليغ البريد ومحضّر التصريح والموقّع، وسطور السنة الميلادية ١٠٠–١٩٠ (مجموع الفصول الأربعة — يطابق نماذج ر10 بالمليم) و٢٥١–٢٩٠ من monthly_salaries حصراً<?= $ofFilterTitle !== '' ? ' — الفلتر: ' . e($ofFilterTitle) : '' ?>.</p>
        </div>
    </div>
<?php
    include __DIR__ . '/../includes/footer.php';
...
<?php elseif ($form === 'cnss_annual'): // 386 التصريح الاسمي السنوي (مؤسسة)
    ...
    $mq = $db->prepare("SELECT ms.month,
                            ...
                            SUM(" . extraWageUsdSql('ms.') . ") ex_usd, SUM(FLOOR(ms.aide_complementaire_lbp/NULLIF(ms.exchange_rate,0))) ai_usd
                        FROM employees e JOIN monthly_salaries ms ON ms.employee_id=e.id
...
<div class="official-doc rtl tcard" id="ppExportArea">
    ...
    <div class="info-grid">
        ...
        <div class="full" style="background:#eef2ff;padding:4px 8px;border-radius:6px"><span class="k">الراتب المركّب (<?= e(salaryCompLabel()) ?>):</span> <strong><?= $sal ? dualFromUsd(composedSalaryLbp($sal), composedSalaryUsd($sal)) : formatLBP($salary) ?></strong></div>
    </div>
    ...
    <table class="doc-table">
        <thead>
            <tr>
                ...
                <th rowspan="2">خاضع للضريبة</th><th rowspan="2">أساس الراتب</th><?= extraAideHeads(' rowspan="2"', $rows, null, null, $sy) ?><?= transportHead(' rowspan="2"') ?><th rowspan="2">الراتب</th><th rowspan="2">توقيع</th>
            </tr>
            <tr><th>حضانة</th><th>ابتدائي</th><th>متوسط</th><th>ثانوي</th></tr>
        </thead>
        <tbody>
        <?php $ttG = ['base'=>0,'base_u'=>0.0,'ex'=>0,'ex_u'=>0.0,'ai'=>0,'ai_u'=>0.0,'tr'=>0,'tr_u'=>0.0,'sal'=>0,'sal_u'=>0.0];
        foreach ($rows as $i=>$r):
            ...
            $ttRate = $sal ? rowRate($sal) : getExchangeRate();
            $ttG['base'] += $sal ? (int)$sal['base_salary_lbp'] : 0; $ttG['base_u'] += 0; // 📄 الأساس بالليرة فقط كما بالبطاقة
            $ttG['ex']   += $sal ? extraWageLbp($sal) : 0;  $ttG['ex_u'] += $sal ? extraWageUsd($sal) : 0;
            ...
            $ttG['sal']  += $rsal; $ttG['sal_u'] += $sal ? lawUsdRow($sal, 'bpe', $rsal) : 0;
        ?>
            <tr>
                <td><?= $i+1 ?></td>
                ...
                <td class="num"><?= moneyLaw($rsal, ['withCur'=>false], $sal ?: null, 'bpe') ?></td>
                <td>&nbsp;</td>
            </tr>
        <?php endforeach; ?>
        ...
        <?php if ($rows): ?><tfoot><tr class="total-row">
            ...
            <td class="num"><?= dualLaw($ttG['base'],$ttG['base_u'],false) ?></td>
            <?= extraAideTotalCells($ttG['ex'],$ttG['ex_u'],$ttG['ai'],$ttG['ai_u']) ?>
            <?= transportTotalCell($ttG['tr'],$ttG['tr_u']) ?>
            <td class="num"><strong><?= dualLaw($ttG['sal'],$ttG['sal_u'],false) ?></strong></td>
            <td></td>
        </tr></tfoot><?php endif; ?>
    </table>
...
<div class="official-doc rtl land-report" id="ppExportArea" style="max-width:100%">
    ...
    <table class="doc-table">
        ...
        foreach ($rows as $i=>$r):
            ...
            $esG['ex']   += $sal ? extraWageLbp($sal) : 0;  $esG['ex_u'] += $sal ? extraWageUsd($sal) : 0;
            ...
            $esG['comp_u'] += $sal ? composedSalaryUsd($sal) : 0;
...
<div class="official-doc rtl" id="ppExportArea">
    ...
    <div class="info-grid">
        ...
        <?php if (salaryCompHas('extra')): ?><div><span class="k">الأجر الإضافي:</span> <strong><?= ($sal ? extraWageMoney($sal) : money(0)) ?></strong></div><?php endif; ?>
        <?php if (salaryCompHas('aide')): ?><div><span class="k">مكافأة ومساعدة:</span> <strong><?= money($sal ? aideCompLbp($sal) : 0, $sal ? rowRate($sal) : null) ?></strong></div><?php endif; ?>
        <div style="background:#eef2ff;padding:4px 8px;border-radius:6px"><span class="k">الراتب المركّب (<?= e(salaryCompLabel()) ?>):</span> <strong><?= $sal ? dualFromUsd(composedSalaryLbp($sal), composedSalaryUsd($sal)) : formatLBP($base) ?></strong></div>
...
<?php elseif ($form === 'eoc_quarterly'):
    ...
    // عمود «الأجر الإضافي» يتبع زرّ «الراتب يشمل» (طلب المستخدم p1) — المحسومات نفسها تبقى المخزّنة القانونية
    $allSpan = ($multiS ? 11 : 10) + (salaryCompHas('extra') ? 1 : 0);
?>
    <form method="get" class="card no-print">
    ...
    <table class="doc-table">
        ...
        <?php $totPrev=0;$totCur=0;$totM=0;$totQ=0;$totHalf=0;$totMisc=0;$totEx=0; foreach ($rows as $i=>$r):
            ...
            <tr>
                ...
                <?php if (salaryCompHas('extra')): ?><td class="num"><?= $exM>0?formatLBP($exM,false):'00' ?></td><?php endif; ?>
    ...
    <table class="doc-table">
        ...
        $drawTotal = function($label, $a, $isGrand) {
            ...
            <tr class="<?= $cls ?>" style="<?= $bg ?>font-weight:700"><td colspan="2" style="text-align:right"><?= e($label) ?></td>
                <td class="num"><?= dualLaw($a['base'],$a['base_usd'],false) ?></td><td class="num"><?= dualLaw($a['ech'],$a['ech_usd'],false) ?></td>
                <td class="num"><?= dualLaw($a['bpe'],$a['bpe_usd'],false) ?></td>
        ...
        foreach ($rows as $r):
            ...
            foreach (['caisse','eocg','fded','txb','tax','cnss','ded','fam','due','net'] as $uk) $add[$uk.'_usd'] = lbpToUsd((float)$add[$uk], $rRate);
            foreach (['base','ech','bpe'] as $uk) $add[$uk.'_usd'] = lawUsdRow($r, $uk, $add[$uk]); // 📄 كما بالبطاقة
            ...
            <tr><td><?= ++$nn ?></td>
                ...
                <td class="num"><?= moneyLaw($r['echelon_value_lbp'], ['withCur'=>false]) ?></td>
                <td class="num"><?= moneyLaw($r['base_plus_echelon_lbp'], ['withCur'=>false], $r, 'bpe') ?></td>
                <?= extraAideCells($r) ?>
...
<?php elseif ($form === 'differences'):
    ...
    $q = $db->prepare("SELECT e.id, e.employee_type, e.first_name_ar, e.last_name_ar, e.first_name_fr, e.last_name_fr,
                   ...
                   SUM(CASE WHEN ms.school_year=? THEN " . extraWageUsdSql('ms.') . " ELSE 0 END) AS extra_wage_usd,
                   ...
                   SUM(CASE WHEN ms.school_year=? AND " . pctLawHolderInSql((string)($schoolYear ?? $sy ?? activeSchoolYear())) . " THEN " . lawUsdSql('ms.base_plus_echelon_lbp') . " ELSE 0 END) AS bpe_usd,
                   ...
                   SUM(CASE WHEN ms.school_year=? THEN 0 ELSE 0 END) AS base_salary_usd,
                   SUM(CASE WHEN ms.school_year=? THEN FLOOR(ms.net_salary_lbp/NULLIF(ms.exchange_rate,0)) ELSE 0 END) AS cur_usd,
...
<div class="official-doc rtl land-report" id="ppExportArea" style="max-width:100%">
    ...
    <table class="doc-table">
        <thead><tr><th>#</th><th>الاسم والشهرة</th><th>أساس الراتب</th><?= extraAideHeads('', $rows, null, null, $schoolYear) ?><th style="background:#4338ca">الراتب المركّب<br><small style="font-weight:400"><?= e(salaryCompLabel()) ?></small></th><th>صافي <?= e($prevSY) ?></th><th>صافي <?= e($schoolYear) ?></th><th>الفرق</th></tr></thead>
        <tbody>
        <?php
        ...
        $drawTotal = function($label,$a,$isGrand){
            $bg=$isGrand?'':'background:#e0e7ff;'; $cls=$isGrand?'total-row':'subtotal-row'; $d=$a['cur']-$a['prev']; $du=$a['cur_usd']-$a['prev_usd']; ?>
            <tr class="<?= $cls ?>" style="<?= $bg ?>font-weight:700"><td colspan="2" style="text-align:right"><?= e($label) ?></td>
                <td class="num"><?= dualLaw($a['base'],$a['base_usd'],false) ?></td>
                <?= extraAideTotalCells($a['ex'],$a['ex_usd'],$a['ai'],$a['ai_usd']) ?>
                <td class="num" style="background:#eef2ff"><strong><?= dualFromUsd($a['composed'],$a['composed_usd'],false) ?></strong></td>
                <td class="num"><?= dualFromUsd($a['prev'],$a['prev_usd'],false) ?></td><td class="num"><?= dualFromUsd($a['cur'],$a['cur_usd'],false) ?></td>
                <td class="num" style="color:<?= $d>=0?'#15803d':'#b91c1c' ?>"><strong><?= ($d>=0?'+':'').dualFromUsd($d,$du,false) ?></strong></td></tr>
        <?php };
        ...
        foreach ($rows as $r):
            ...
            <tr><td><?= ++$nn ?></td>
                ...
                <td class="num"><?= dualFromUsd($r['prev'],(float)$r['prev_usd'],false) ?></td>
                <td class="num"><?= dualFromUsd($r['cur'],(float)$r['cur_usd'],false) ?></td>
                <td class="num" style="color:<?= $diff>=0?'#15803d':'#b91c1c' ?>"><strong><?= ($diff>=0?'+':'').dualFromUsd($diff,$diffU,false) ?></strong></td></tr>
        <?php endforeach;
        if ($rows && $curCat !== null) $drawTotal('مجموع '.empCategoryTitle($curCat), $sub, false);
...
<?php elseif ($form === 'general_report'):
    ...
    $q = $db->prepare("SELECT ms.school_id,
            ...
            SUM(CASE WHEN " . pctLawHolderInSql((string)($schoolYear ?? $sy ?? activeSchoolYear())) . " THEN " . lawUsdSql('ms.base_plus_echelon_lbp') . " ELSE 0 END) bpe_usd,
            ...
            SUM(FLOOR((ms.cnss_amount_lbp + ms.school_cnss_8_lbp)/NULLIF(ms.exchange_rate,0))) cnss_usd,
    ...
    <table class="doc-table" style="margin-top:8px">
        ...
        ?>
            <tr>
                ...
                <td class="num"><?= dualLaw($bpe, $bpeu, false) ?></td>
        ...
        <tfoot><tr class="total-row">
            ...
            <td class="num"><?= dualLaw($T['bpe'],$T['bpe_usd'],false) ?></td>
...
<div class="official-doc cnss-form rtl" id="ppExportArea">
    ...
    <table class="doc-table" style="max-width:680px;margin:6px auto">
        ...
        <?php for ($i=0;$i<6;$i++): $p=$paid[$i] ?? null; ?>
            <tr><td><?= $p?monthName($p['month'],'ar').' '.$p['year']:'&nbsp;' ?></td><td>&nbsp;</td><td class="num"><?= $p?extraWageMoney($p,['withCur'=>false]):'' ?></td><td class="num"><?= $p?money(aideCompLbp($p),rowRate($p),['withCur'=>false]):'' ?></td><td class="num"><?= $p?formatLBP(cnssSubjectWageLbp($p,$emp),false):'' ?></td></tr>
        <?php endfor; ?>
        </tbody>
    </table>
    ...
    <table class="doc-table">
        ...
        foreach ($rows as $r):
            ...
            $add=['base'=>(int)$r['base_salary_lbp'],'ex'=>extraWageLbp($r),'ai'=>aideCompLbp($r),'trans'=>(int)$r['transport_lbp'],'net'=>(int)$r['net_salary_lbp'],'fam'=>(int)$r['family_allowance_lbp'],'due'=>dueShownLbp($r),
                  ...
                  'composed'=>composedSalaryLbp($r),'composed_usd'=>composedSalaryUsd($r),
                  'base_usd'=>0.0,'net_usd'=>lbpToUsd((int)$r['net_salary_lbp'],$rRate),'fam_usd'=>lbpToUsd((int)$r['family_allowance_lbp'],$rRate),'due_usd'=>lbpToUsd(dueShownLbp($r),$rRate)];
            foreach ($add as $k=>$v){ $G[$k]+=$v; $sub[$k]+=$v; } ?>
            <tr>
                <td><?= ++$nn ?></td>
    ...
    <table class="doc-table">
        ...
        $drawTotal = function($label,$a,$isGrand) use ($csLbl){
            ...
            <tr class="<?= $cls ?>" style="<?= $bg ?>font-weight:700"><td colspan="<?= $csLbl ?>" style="text-align:right"><?= e($label) ?></td>
                <td class="num"><?= dualLaw($a['base'],$a['base_usd'],false) ?></td><td class="num"><?= dualLaw($a['ech'],$a['ech_usd'],false) ?></td><td class="num"><?= dualLaw($a['bpe'],$a['bpe_usd'],false) ?></td>
                <?= extraAideTotalCells($a['extra'],$a['extra_usd'],$a['aide'],$a['aide_usd']) ?>
                <td class="num" style="background:#eef2ff"><strong><?= dualFromUsd($a['composed'],$a['composed_usd'],false) ?></strong></td>
        ...
        foreach ($rows as $r):
            ...
            foreach (['base','ech','bpe'] as $uk) $add[$uk.'_usd'] = lawUsdRow($r, $uk, $add[$uk]); // 📄 كما بالبطاقة // دولار القانون ÷1500
            ...
            <tr>
                ...
                <?= extraAideCells($r) ?>
                <td class="num" style="background:#eef2ff"><strong><?= dualFromUsd(composedSalaryLbp($r), composedSalaryUsd($r), false) ?></strong></td>
                <td class="num"><?= money($r['school_cnss_8_lbp'], $rRate, ['withCur'=>false]) ?></td>
                <td class="num"><?= money($r['school_eoc_6_lbp'], $rRate, ['withCur'=>false]) ?></td>
                <td class="num"><?= money($r['school_end_of_service_8_5_lbp'], $rRate, ['withCur'=>false]) ?></td>
...
<?php elseif ($form === 'employer_cost'):
    ...
    $q = $db->prepare("SELECT
            ...
            0 baseS_usd,
            SUM(CASE WHEN " . pctLawHolderInSql((string)($schoolYear ?? $sy ?? activeSchoolYear()), 'employee_id') . " THEN " . lawUsdSql('base_plus_echelon_lbp') . " ELSE 0 END) bpe_usd,
            ...
            SUM(FLOOR(transport_lbp/NULLIF(exchange_rate,0))) transport_usd,
            SUM(FLOOR(school_cnss_8_lbp/NULLIF(exchange_rate,0))) cnss8_usd,
            SUM(FLOOR(school_eoc_6_lbp/NULLIF(exchange_rate,0))) eoc6_usd,
            SUM(FLOOR(school_family_comp_6_lbp/NULLIF(exchange_rate,0))) fam6_usd,
            SUM(FLOOR(school_end_of_service_8_5_lbp/NULLIF(exchange_rate,0))) eos85_usd
        FROM monthly_salaries ms JOIN employees e ON e.id=ms.employee_id
    ...
    $famU=(float)($g['family_usd']??0); $transU=(float)($g['transport_usd']??0);
    $cnss8U=(float)($g['cnss8_usd']??0); $eoc6U=(float)($g['eoc6_usd']??0); $fam6U=(float)($g['fam6_usd']??0); $eos85U=(float)($g['eos85_usd']??0);
    $employerCharges = $cnss8+$eoc6+$fam6+$eos85;
    $employerChargesU = $cnss8U+$eoc6U+$fam6U+$eos85U;
    // «الأرقام تركب» + زرّ «الراتب يشمل»: الإجمالي والكلفة = مجموع البنود الظاهرة فقط
    $gross = (int)($g['bpe']??0)
    ...
    $totalCostU = $grossU+$famU+(salaryCompHas('transport') ? $transU : 0)+$employerChargesU;
    $grossParts = ['أساسي + درجة'];
    if (salaryCompHas('extra')) $grossParts[] = 'إضافي';
    if (salaryCompHas('aide'))  $grossParts[] = 'مكافآت';
    $lines = [
        ['الرواتب الإجمالية (' . implode(' + ', $grossParts) . ')', $gross, false, $grossU],
        ['— منها: أساس الراتب', (int)($g['baseS']??0), true, (float)($g['baseS_usd']??0)],
        ['— منها: الراتب بعد التدرّج', (int)($g['bpe']??0), true, (float)($g['bpe_usd']??0)],
    ];
    if (salaryCompHas('extra')) $lines[] = ['— منها: الأجر الإضافي', (int)($g['extraWage']??0), true, (float)($g['extraWage_usd']??0)];
    if (salaryCompHas('aide'))  $lines[] = ['— منها: مكافأة ومساعدة', (int)($g['aideC']??0), true, (float)($g['aideC_usd']??0)];
    $lines[] = ['التعويضات العائلية', $fam, false, $famU];
    if (salaryCompHas('transport')) $lines[] = ['تعويضات النقل', $trans, false, $transU];
    $lines = array_merge($lines, [
        ['— مساهمة المؤسسة بالضمان (٨٪ مرض)', $cnss8, true, $cnss8U],
        ['— مساهمة المؤسسة بنهاية الخدمة (٨.٥٪)', $eos85, true, $eos85U],
        ['— مساهمة المؤسسة بالتعويضات العائلية (٦٪)', $fam6, true, $fam6U],
        ['— مساهمة المؤسسة بصندوق التعويضات (٦٪)', $eoc6, true, $eoc6U],
        ['مجموع مساهمات المؤسسة', $employerCharges, false, $employerChargesU],
...
<div class="official-doc <?= $ofDir ?>" id="ppExportArea" dir="<?= $ofDir ?>">
    ...
    <table class="doc-table" style="max-width:620px;margin:14px auto">
        ...
        <?php foreach ($lines as $ln): $isTot = ($ln[0]==='مجموع مساهمات المؤسسة' || $ln[0]==='الكلفة الإجمالية على المؤسسة'); ?>
            <tr class="<?= $isTot?'total-row':'' ?>"><td style="text-align:right;<?= $ln[2]?'padding-right:24px;color:#475569':'' ?>"><?= e($ln[0]) ?></td><td class="num"><?= (strpos($ln[0], 'أساس الراتب') !== false || strpos($ln[0], 'الراتب بعد التدرّج') !== false) ? dualLaw($ln[1], $ln[3] ?? 0.0, false) : dualFromUsd($ln[1], $ln[3] ?? 0.0, false) ?></td></tr>
        <?php endforeach; ?>
        </tbody>
    </table>
    <div class="doc-note">الكلفة الإجمالية = الرواتب + التعويضات العائلية<?= salaryCompHas('transport') ? ' + النقل' : '' ?> + كل مساهمات المؤسسة (الضمان + نهاية الخدمة + العائلية + صندوق التعويضات) — البنود بحسب زرّ «الراتب يشمل».</div>
    <div class="sign-row"><?= signatureBox('المحاسب', '', '') ?><?= signatureBox('المدير', $school['ville'] ?? '', formatDate(date('Y-m-d'))) ?></div>
</div>
...
<div class="official-doc rtl land-report" id="ppExportArea" style="max-width:100%">
    ...
    <table class="doc-table">
        <thead><tr>
            ...
            <th>تاريخ الدخول</th><th>تاريخ الترك</th><th>رقم الصندوق</th><th>الرقم المالي</th><th>الراتب<?php if (compColsCount()): ?><br><small style="font-weight:400"><?= e(salaryCompLabel()) ?></small><?php endif; ?></th>
        </tr></thead>
        <tbody>
        <?php $curCat=null; $giTot=0; foreach ($rows as $i=>$r): echo categoryHeaderRow($curCat, $r['employee_type'], 9);
            ...
            $giRate = $sal ? rowRate($sal) : getExchangeRate();
            $giTot += $rsal; $giTotUsd = ($giTotUsd ?? 0.0) + lbpToUsd($rsal, $giRate);
        ?>
            <tr>
                <td><?= $i+1 ?></td>
                <td style="text-align:right"><?= e(trim($r['first_name_ar'].' '.$r['last_name_ar']) ?: ($r['first_name_fr'].' '.$r['last_name_fr'])) ?></td>
        ...
        <?php if ($rows): ?><tfoot><tr class="total-row">
            ...
            <td class="num"><strong><?= dualFromUsd($giTot, $giTotUsd ?? 0.0, false) ?></strong></td>
        </tr></tfoot><?php endif; ?>
    </table>
    <div style="margin-top:10px;font-weight:700">العدد الإجمالي: <?= count($rows) ?></div>
    <div class="sign-row"><?= signatureBox('توقيع المدير وخاتم المدرسة', $school['ville'] ?? '', formatDate(date('Y-m-d'))) ?></div>
...
<?php elseif ($form === 'cnss_nominative_monthly'):
    ...
    // المجاميع
    $T = ['baseSal'=>0,'teach'=>0,'cola'=>0,'bonus'=>0,'trans'=>0,'work'=>0,'base'=>0,'nT'=>0,'nW'=>0,
          'm3'=>0,'m8'=>0,'mtot'=>0,'eos'=>0,'fam6'=>0,'due'=>0,'fpaid'=>0,'rest'=>0];
    // أعمدة الإضافي/المكافأة/النقل تتبع زرّ «الراتب يشمل» بالترويسة (كباقي الكشوف) —
    // عمود «مجموع الرواتب ضمن الحد الأقصى» يبقى وعاء الضمان القانوني المخزّن كما هو.
    $nomCols = 19 + compColsCount();
?>
    <form method="get" class="card no-print">
...
<div class="official-doc rtl land-report" id="ppExportArea" style="max-width:100%">
    ...
    <table class="doc-table">
        ...
        <?php $curCat=null; foreach ($rows as $i=>$r):
            ...
            echo categoryHeaderRow($curCat, $type, $nomCols);
            ...
            $trans = (int)$r['transport_lbp'];                          // تعويض النقل (عرض فقط — ليس من وعاء الضمان)
        ...
        <?php endforeach; ?>
        <?php if(!$rows): ?><tr><td colspan="<?= $nomCols ?>" class="text-center">لا يوجد مضمونون لهذا الشهر</td></tr><?php endif; ?>
        </tbody>
        <?php if ($rows): ?>
        <tfoot>
            <tr class="total-row">
...
<?php elseif ($form === 'salary_detail'):
    ...
    $keys = ['base','ech','bpe','bpe_usd','cola','cola_usd','bonus','composed','composed_usd','half','caisse','gross','fded','txb','tax','cnss','ret','net','fam','trans','due'];
    ...
    $rowVals = function($r) use ($month, $year) {
        ...
        return [
            'base'=>(int)$r['base_salary_lbp'], 'ech'=>(int)$r['echelon_value_lbp'], 'bpe'=>(int)$r['base_plus_echelon_lbp'], 'bpe_usd'=>(int)lawUsdRow($r, 'bpe'), // 📄 كما بالبطاقة
...
<div class="official-doc <?= $ofDir ?> land-report" id="ppExportArea" dir="<?= $ofDir ?>" style="max-width:100%">
    ...
    <table class="doc-table">
        ...
        <?php
        // صفّ بيانات
        $drawRow = function($n, $name, $v) use ($fmt, $fmtEx, $fmtL) { ?>
            <tr>
                <td><?= $n ?></td>
                <td style="text-align:right;white-space:nowrap"><?= $name /* HTML جاهز من ofStateNameCell */ ?></td>
                <td class="num"><?= $fmtL($v['base']) ?></td>
                <td class="num"><?= $fmtL($v['ech']) ?></td>
                <td class="num"><?= (int)$v['bpe'] ? dualLaw($v['bpe'], $v['bpe_usd'], false) : '0' ?></td>
                <?php if (salaryCompHas('extra')): ?><td class="num"><?= $fmtEx($v['cola'], $v['cola_usd']) ?></td><?php endif; ?>
                <?php if (salaryCompHas('aide')): ?><td class="num"><?= $fmt($v['bonus']) ?></td><?php endif; ?>
                <td class="num" style="background:#eef2ff"><strong><?= $fmtEx($v['composed'], $v['composed_usd']) ?></strong></td>
        ...
        <?php };
        // صفّ مجموع
        $drawTotal = function($label, $a) use ($fmt, $fmtEx, $fmtL) { ?>
            <tr class="total-row">
                <td></td><td style="text-align:right"><?= e($label) ?></td>
                <td class="num"><?= $fmtL($a['base']) ?></td><td class="num"><?= $fmtL($a['ech']) ?></td>
                <td class="num"><?= (int)$a['bpe'] ? dualLaw($a['bpe'], $a['bpe_usd'], false) : '0' ?></td>
                <?php if (salaryCompHas('extra')): ?><td class="num"><?= $fmtEx($a['cola'], $a['cola_usd']) ?></td><?php endif; ?>
        ...
        foreach ($rows as $r):
            ...
            if ($cat !== $curCat):
                ...
                $sub = $zero; $curCat = $cat;
                ?><tr><td colspan="<?= $sdCols ?>" style="text-align:right;font-weight:700;background:#eef2ff"><?= e($catLabel[$cat] ?? $cat) ?></td></tr><?php
            endif;
            $v = $rowVals($r);
