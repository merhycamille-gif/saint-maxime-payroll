...
if ($report === 'monthly_summary') {
    ...
    $st = $db->prepare("SELECT e.employee_type,e.first_name_fr,e.last_name_fr,e.first_name_ar,e.last_name_ar,e.finance_ministry_number,e.school_id,e.social_status,e.spouse_works,e.payment_months_per_year,COALESCE(e.apply_family_deduction,1) afd,COALESCE(e.grant_spouse_addition,1) gsa,COALESCE(e.grant_children_addition,0) gca,ms.base_salary_lbp,ms.base_plus_echelon_lbp,ms.transport_lbp,ms.income_tax_lbp,ms.taxable_base_lbp,ms.extra_lbp,ms.prime_fixe_lbp,ms.aide_complementaire_lbp
        FROM monthly_salaries ms JOIN employees e ON e.id=ms.employee_id
    ...
    $fdOf = function ($r) use ($fdAsOf) {
        ...
        return (int)round(familyDeductionAnnual($r['social_status'] ?? '', $r['spouse_works'] ?? 0, $r['afd'] ?? 1, $fdAsOf, $r['gsa'] ?? 1, $r['gca'] ?? 0) / 12);
    };
    ...
    if (salaryCompHas('aide'))  $head[] = 'مكافأة ومساعدة';
    $head[] = 'الراتب المركّب (' . salaryCompLabel() . ')';
    $head = array_merge($head, ['التنزيل العائلي (حصّة الشهر)', 'الراتب الخاضع (بعد حسم التنزيل)', 'ضريبة الدخل']);
    $rep->head($head);
    $z = ['base' => 0, 'extra' => 0, 'aide' => 0, 'composed' => 0, 'txb' => 0, 'fded' => 0, 'tax' => 0]; $G = $z; $cur = null; $sub = $z; $subN = 0; $rn = 0;
    $pad = $schCol ? 4 : 3;
    $emit = function ($label, $a, $n) use ($rep, $pad) {
        $row = array_fill(0, $pad, ''); $row[$pad - 1] = 'مجموع ' . $label . ' — العدد: ' . $n;
        $row[] = $a['base'];
        if (salaryCompHas('extra')) $row[] = $a['extra'];
        if (salaryCompHas('aide'))  $row[] = $a['aide'];
        $row = array_merge($row, [$a['composed'], $a['fded'], $a['txb'], $a['tax']]);
        $rep->totalRow($row);
    };
    foreach ($data as $r) {
        if ($cur !== null && $r['employee_type'] !== $cur) { $emit($catTitle($cur), $sub, $subN); $sub = $z; $subN = 0; }
