...
if ($form === 'cnss_taswiya') {
    ...
    if ($extraBlocks > 0) {
        ...
        // 🎨 «ليش عم يطلع هيك التابلو» (p1 2026-08-26): النسخ الحرفي لصفوف الكتلة الثانية كان
        // يرث رقعها التنسيقية (صفوف s=253/255 وبولد شاذ بصف 7878093) فتطلع الكتل الإضافية
        // مرقّعة. الحل: صف نموذجي واحد موحّد (الصف 30 — الستايل الأكثر شيوعاً s=4) مفرّغ من
        // القيم والصيَغ، يتكرر لكل صفوف البيانات المستنسخة؛ وسطر «المجموع» من صف 47 كما هو.
        $rowTpl = preg_replace(['#<v>[^<]*</v>#', '#<is>.*?</is>#s'], '', $stripF($grab(30)));
        $subTpl = $stripF($grab(47)); // يبقي «المجموع» بخانة الاسم وقيمه تُكتب فوقها
        $clones = '';
        for ($i = 0; $i < $extraBlocks; $i++) {
            for ($r = 0; $r < 19; $r++) {
                $clones .= $renum($rowTpl, 30, 30, (48 + 20 * $i + $r) - 30);
            }
            $clones .= $renum($subTpl, 47, 47, 20 + 20 * $i);
        }
    ...
    foreach ($dataRows as $idx => $rn) {
        ...
        if ($inClone && $p) {
            ...
            $ag['A'] += ($p['R'] > 0 ? 1 : 0); $ag['D'] += (($p['worker'] && $p['O'] > 0) ? 1 : 0); // ⚖️ الخاضعون لنهاية الخدمة فقط (2026-09-26)
