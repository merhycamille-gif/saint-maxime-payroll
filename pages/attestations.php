...
<?php else:
    ...
    // 🚪 «إذا عندو الأستاذ ترك لازم نحطّو + خيار غيّره أو شيله» (2026-09-24): الافتراضي تاريخ الترك من ملفه
    //    (ترك من الكل، وإلا الضمان/المالية/الصندوق)؛ end_dt يغيّره لهذه الإفادة، وend_none يشيله (خطّ منقّط).
    //    بلا تاريخ ترك ⇒ الإفادات التي تحتاج «إلى» تطبع تاريخ الإفادة كما قبل، وإفادتا الراتب/العمل تبقيان «ولا يزال حتى تاريخه».
    $leftFile = '';
    foreach (['left_date_all', 'left_date_cnss', 'left_date_finance', 'left_date_eoc'] as $lc) {
        $lv = (string)($emp[$lc] ?? '');
        if (preg_match('/^\d{4}-\d{2}-\d{2}$/', $lv) && strtotime($lv) && $lv > '1900-01-01') { $leftFile = $lv; break; }
    }
    $endNone = !empty($_GET['end_none']);
    $endDate = (string)($_GET['end_dt'] ?? '');
    if (!preg_match('/^\d{4}-\d{2}-\d{2}$/', $endDate) || !strtotime($endDate)) $endDate = '';
    if ($endNone) $endDate = '';
    elseif ($endDate === '') $endDate = $leftFile;
    $endActive = ($endDate !== ''); // الأستاذ تارك (من الملف أو بيد المستخدم) ⇒ «من … ولغاية …» بدل «ولا يزال حتى تاريخه»
    // 📚 «المادة أو المواد اللي بيعلّمها لازم يكون عندي خيار أكتب أو غيّر» (2026-09-24): تحلّ محلّ مادة الملف بهذه الإفادة فقط
    $subjOvr = trim((string)($_GET['subj_ovr'] ?? ''));
    if ($subjOvr !== '') $emp['subjects_taught'] = $subjOvr;
    ...
    // 🧑 صيغة المذكّر/المؤنّث بكل الإفادات من خانة الجنس بملف الموظف («p1 شوف هيك صح شي» على إفادة راتبه
    // كميل مرعي 2026-09-15: كان النصّ «السيّد(ة)… يعمل(تعمل)… طلبه(ا)» رغم أنّ جنسه محدَّد بملفه):
    // معروف ⇒ صيغة واحدة نظيفة (السيّد… يعمل… مدرّس… طلبه / السيّدة… تعمل… مدرّسة… طلبها)؛
    ...
    // 📚 «شيّك بالإفادات إذا عم تذكر المواد اللي بيدرّسها» (2026-09-24): الإفادات القانونية (الضمان/المدرسية/الإسقاط/الإبراء/
    //    الإقرار/العقد-المتعاقد/القسم العام) تذكر الصفة وحدها ⇒ الصفة + « — مادة X» للأستاذ ذي المادة (الموظف الإداري كما هو).
    $subjAll = trim((string)($emp['subjects_taught'] ?? ''));
    $fnSubj = $fnFr;
    if (!$isEmploye && $subjAll !== '') {
        $fnSubj['ar'] = $fnFr['ar'] . ' — مادة ' . subjectToLang($subjAll, 'ar');
        $fnSubj['fr'] = $fnFr['fr'] . ' — matière : ' . subjectToLang($subjAll, 'fr');
        $fnSubj['en'] = $fnFr['en'] . ' — subject: ' . subjectToLang($subjAll, 'en');
    }
    ...
    // 🗓️ «انتبه أنا حاطط مثلاً مع نقل ما عم يحطها» (2026-09-15، إفادة كميل مرعي بتاريخ 15/9/2026 والسنة
    // المختارة 2026-2027): كان يأخذ **آخر شهر بالسنة** (أيلول 2027 — صيفي بلا نقل) فيطلع النقل 0 ولو
    // أُشِّر خياره. صار: بين الأشهر ذات الإضافي/الصافي نأخذ آخر شهر **لا يتجاوز تاريخ الإفادة**، وإن كانت
    // أشهر السنة كلها بعد التاريخ (سنة لم تبدأ) فأوّل شهر فيها (تشرين الأول — شهر تدريس بنقله).
    $attYm = (int)date('Ym', strtotime($effDate) ?: time());
    $salPickSql = "ORDER BY (prime_fixe_lbp > 0 OR extra_lbp > 0) DESC,
                 (net_salary_lbp > 0 OR base_plus_echelon_lbp > 0) DESC,
                 (year*100+month <= $attYm) DESC,
                 CASE WHEN year*100+month <= $attYm THEN -(year*100+month) ELSE (year*100+month) END ASC LIMIT 1";
    $sal = null;
    ...
    $yAr=$yFr=$yEn='';
    if ($emp['hire_date']) {
        $diff=(new DateTime($emp['hire_date']))->diff(new DateTime($endDate !== '' ? $endDate : $effDate)); // من الدخول إلى الترك (2026-09-24)
    ...
    $qs = 'employee_id='.$employeeId.'&type='.urlencode($type).'&date='.urlencode($effDate).'&opts_set=1'.($incExtra?'&inc_extra=1':'').($incAide?'&inc_aide=1':'').($incTrans?'&inc_trans=1':'').'&cur='.$cur.($eos>0?'&eos='.$eos:'').($isqMode?'&isq='.$isqMode:'').'&logo='.($showLogo?'1':'0').($isNotice?'&subj_txt='.urlencode($subjectTxt):'').($type==='riaaya'?'&assoc_txt='.urlencode($assocTxt):'').($embAmt>0?'&emb_amt='.$embAmt:'').($embCur!=='usd'?'&emb_cur='.$embCur:'').($embPer!=='month'?'&emb_per='.$embPer:'').($grant>0?'&grant='.$grant:'').($aqdLbp>0?'&aqd_lbp='.$aqdLbp:'').($aqdUsd>0?'&aqd_usd='.$aqdUsd:'').($sigIdx>0?'&sig='.$sigIdx:'').($hasSigTitle?'&sig_t='.$sigTitle:'').($lvSel!==''?'&lv_sel='.urlencode($lvSel):'').($lvTxt!==''?'&lv_txt='.urlencode($lvTxt):'')
        ...
        .($hireOvr!==''?'&hire_dt='.$hireOvr:'').($endDate!==''?'&end_dt='.$endDate:'').($endNone?'&end_none=1':'').($subjOvr!==''?'&subj_ovr='.urlencode($subjOvr):'')
...
?>
    ...
    ob_start(); // 🪪 لحقن سطر الأرقام قبل التوقيع (2026-09-24)
    if ($type === 'cnss'):
    ?>
    ...
    <div id="ppExportArea" class="<?= $lhClass ?>" style="<?= $lhStyle ?>" dir="rtl"<?= $type === 'aqd_taalim' ? '' : ' data-fit1="1"' ?>>
        <div class="card-body" style="line-height:2.15;text-align:right;font-size:12pt;font-family:Arial,'Segoe UI',Tahoma,sans-serif;<?= $lhOn?'padding:0':'' ?>">
            ...
            <div dir="ltr" style="text-align:justify">
              ...
              <p><?= $FR ? 'à compter du' : 'as of' ?> <strong><?= $hireFmt ?></strong> <?= $FR ? 'en qualité de' : 'in the capacity of' ?> (<strong><?= e($FR ? $fnSubj['fr'] : $fnSubj['en']) ?></strong>)</p>
            ...
            <p>اعتباراً من تاريخ <strong><?= $hireFmt ?></strong> بصفة (<strong><?= e($fnSubj['ar']) ?></strong>)</p>
    ...
    <?php elseif (in_array($type, ['salaire','tadris','embassy','riaaya','anhaa_khedme','anhaa_mail','talab_istiqala','afade_madrasiya','isqat_haq','baraa_zimma','iqrar','aqd_taalim','notice_school','notice_mail'])):
        ...
        // 📚 p1: «بدنا نكتب لمادة اللغة الإنكليزية» (2026-08-21) — المادة تُكتب بلغة الوثيقة
        // نفسها مهما كانت لغة تخزينها بالملف (Anglais → اللغة الإنكليزية بالعربي، والعكس)
        $subjAr = subjectToLang($subj, 'ar');
        $subjFr = subjectToLang($subj, 'fr');
        $subjEn = subjectToLang($subj, 'en');
    ...
    <div id="ppExportArea" class="<?= $lhClass ?>" style="<?= $lhStyle ?>" dir="rtl"<?= $type === 'aqd_taalim' ? '' : ' data-fit1="1"' ?>>
      ...
      <?php if ($docLang !== 'ar' && $type !== 'embassy'):
        ...
        $funcLatS = $FR ? $fnSubj['fr'] : $fnSubj['en']; // الصفة + المادة للإفادات القانونية (2026-09-24)
        ...
        // 🧑 «إفادة عمل وتدريس مظبوطة لغوياً» (2026-09-24): الضمير بحسب الجنس (Il/Elle ⇒ Il أو Elle، He/She ⇒ He أو She)،
        //    والفعل بالماضي للتارك: a enseigné / a travaillé … du X au Y — taught / worked … from X to Y (بدل enseigne … jusqu'au)
        $heShe   = $FR ? $g('Il', 'Elle', 'Il/Elle') : $g('He', 'She', 'He/She');
        $vTeach  = $endActive ? ($FR ? 'a enseigné au sein de son établissement la matière' : 'taught') : ($FR ? 'enseigne au sein de son établissement la matière' : 'has been teaching');
        $vWork   = $endActive ? ($FR ? 'a travaillé au sein de son établissement en qualité de' : 'worked there as') : ($FR ? 'travaille au sein de son établissement en qualité de' : 'has been working there as');
        $fromW   = $endActive ? ($FR ? 'du' : 'from') : ($FR ? 'depuis le' : 'since');
        $toPart  = $endActive ? (($FR ? ' au ' : ' to ') . '<strong>' . $endFmt . '</strong>') : '';
        $salParts = [];
      ...
      ?>
        ...
        <?php endif; ?>
        <h2 style="text-align:center;margin:6px 0 22px;text-decoration:underline"><?= $FR ? 'Lettre de fin de service' : 'End-of-Service Letter' ?></h2><?= $rateLine ?>
        ...
        <h2 style="text-align:center;margin:6px 0 22px;text-decoration:underline"><?= $FR ? 'Renonciation de droits' : 'Waiver of Rights' ?></h2><?= $rateLine ?>
        <p><?= $FR ? 'Je soussigné(e)' : 'I, the undersigned' ?> <strong><?= e($nomFr) ?></strong></p>
        ...
        <h2 style="text-align:center;margin:6px 0 22px;text-decoration:underline"><?= $FR ? 'Déclaration' : 'Declaration' ?></h2><?= $rateLine ?>
        <p><?= $FR ? 'Je soussigné(e),' : 'I, the undersigned,' ?></p>
        <p><?= $FR ? 'Nom complet' : 'Full name' ?> : <strong><?= e($nomFr) ?></strong></p>
        <p><?= $FR ? 'ai contracté avec l\'école' : 'contracted with' ?> : <strong><?= e($schoolNameFr) ?></strong> <?= $FR ? 'pour l\'année scolaire' : 'for the academic year' ?> : <strong><?= $nextSY ?></strong> <?= $FR ? 'en qualité de' : 'as' ?> <strong><?= e($funcLatS) ?></strong>.</p>
        ...
        <?php endif; ?>
        <h2 style="text-align:center;margin:4px 0 18px;text-decoration:underline"><?= $FR ? 'Avertissement' : 'Warning' ?></h2><?= $rateLine ?>
        ...
        ?>
        <h2 style="text-align:center;margin:4px 0 18px;text-decoration:underline"><?= $FR ? 'Contrat d\'enseignement' : 'Teaching Contract' ?></h2><?= $rateLine ?>
        ...
        <p style="margin-left:26px">ـ <?= $FR ? 'nombre d\'heures d\'enseignement effectif par semaine' : 'number of actual teaching hours per week' ?> : <?= $vb($hpw, 70) ?> &nbsp; <?= $FR ? 'niveau' : 'level' ?> : <?= $vb($niveau, 110) ?> &nbsp; <?= $FR ? 'matière' : 'subject' ?> : <?= $vb($subjL, 110) ?></p>
      ...
      <?php elseif ($type === 'anhaa_khedme'): ?>
        <?php if ($showRecHead): ?><?= $schoolHead ?><?php endif; ?>
        <h2 style="text-align:center;margin:6px 0 22px;text-decoration:underline">كتاب إنهاء خدمات</h2><?= $rateLine ?>
        <p>حضرة <?= $g('الأستاذ', 'الأستاذة', 'الأستاذ(ة)') ?> : <strong><?= e($nomAr) ?></strong></p>
      ...
      <?php elseif ($type === 'anhaa_mail'): ?>
        ...
        <div style="border:1px solid #bbb;padding:8px 12px;line-height:1.95;margin-bottom:14px">
          ...
          <p style="margin:2px 0">إلى <?= $g('الأستاذ', 'الأستاذة', 'الأستاذ(ة)') ?> : <strong><?= e($nomAr) ?></strong></p>
        ...
        <h2 style="text-align:center;margin:4px 0 18px;text-decoration:underline">كتاب إنهاء خدمات</h2><?= $rateLine ?>
        <p>استناداً إلى القوانين المرعية الإجراء وخاصةً قانون الهيئة التعليمية للمدارس الخاصة ، وعملاً بالمادة 29 وتعديلاتها من القانون المذكور ،</p>
        <p>نُعلمكم قبل الخامس من شهر تموز <strong><?= $effYear ?></strong> قرار اضطرار مدرسة : <strong><?= e($schoolNameAr) ?></strong></p>
        <p>إنهاء خدماتكم التعليمية لديها عن العام الدراسي <strong><?= $curSY ?></strong> وما يليه .</p>
        <p>وإذ تأسف إدارة المدرسة لإبلاغكم قرارها تشكر لكم تعاونكم المخلص معها داعيةً لكم بالتوفيق راجيةً قبول احترامها .</p>
      ...
      <?php elseif ($type === 'isqat_haq'): ?>
        <?php if ($showRecHead): ?><?= $schoolHead ?><?php endif; ?>
        <h2 style="text-align:center;margin:6px 0 22px;text-decoration:underline">إفـادة إسقاط حـق</h2><?= $rateLine ?>
        <p>أنا الموقّع أدناه <strong><?= e($nomAr) ?></strong></p>
        <p>حسب تذكرة هويتي ، رقم السجل <?= $blank(150) ?> &nbsp; أُصرّح بما يلي :</p>
        <p><strong>1)</strong> عملت لدى مدرسة : <strong><?= e($schoolNameAr) ?></strong> بصفة : <strong><?= e($fnSubj['ar']) ?></strong> منذ <strong><?= $emp['hire_date'] ? $hireFmt : $blank(110) ?></strong> ، وأصبح راتبي بتاريخ هذا الإسقاط بالغاً ( بالأرقام ) <strong><?= $salFig ?></strong> ، بالحروف <strong><?= e($salWrd) ?> لا غير .</strong></p>
      ...
      <?php elseif ($type === 'iqrar'):
        $iqEndYear = $syStart + 2; ?>
        <?php if ($showRecHead): ?><?= $schoolHead ?><?php endif; ?>
        <h2 style="text-align:center;margin:6px 0 22px;text-decoration:underline">إقــرار</h2><?= $rateLine ?>
        <p>أنا الموقّع أدناه ،</p>
        <p>الاسم الكامل : <strong><?= e($nomAr) ?></strong></p>
        <p>تعاقدت مع مدرسة : <strong><?= e($schoolNameAr) ?></strong> للعام الدراسي : <strong><?= $nextSY ?></strong> بصفة <strong><?= e($fnSubj['ar']) ?></strong> .</p>
      ...
      ?>
        <?php if ($showRecHead): ?><?= $schoolHead ?><?php endif; ?>
        <h2 style="text-align:center;margin:4px 0 18px;text-decoration:underline">عقــد تعليــم</h2><?= $rateLine ?>
        ...
        <p style="margin-right:26px">ـ عدد ساعات التعليم الفعلي في الأسبوع : <?= $vb($hpw, 70) ?> &nbsp; المرحلة: <?= $vb($niveau, 110) ?> &nbsp; المادة: <?= $vb($subjAr, 110) ?></p>
      ...
      <?php elseif ($type === 'notice_school'): ?>
        <?php if ($showRecHead): ?><?= $schoolHead ?><?php endif; ?>
        <p>حضرة <?= $g('الأستاذ', 'الأستاذة', 'الأستاذ(ة)') ?> : <strong><?= e($nomAr) ?></strong> <?= $g('المحترم', 'المحترمة', 'المحترم(ة)') ?> .</p>
        ...
        <div style="margin-top:30px;border-top:1px dashed #999;padding-top:10px">
          ...
          <p><?= $g('الأستاذ', 'الأستاذة', 'الأستاذ(ة)') ?> : <?= $blank(220) ?></p>
    ...
    <div id="ppExportArea" class="card" style="max-width:820px;margin:0 auto;padding:10px" data-fit1="1" <?= $rtl?'dir="rtl"':'' ?>>
        <div class="card-body" style="line-height:1.95;font-size:12pt;font-family:Arial,'Segoe UI',Tahoma,sans-serif;<?= $rtl?'text-align:right':'' ?>">
            ...
            <?php if ($docLang === 'ar'): ?>
                ...
                <p><strong><?= e($nomAr) ?></strong>، يشغل وظيفة <strong><?= e($fnSubj['ar']) ?></strong><?php if ($emp['hire_date']): ?>، منذ تاريخ <strong><?= $hireFmt ?></strong><?php endif; ?><?php if ($isTit && $emp['titularization_date']): ?>، تاريخ الترسيم <strong><?= $titFmt ?></strong><?php endif; ?>، من ضمن ملاك مدرستنا<?= $emp['status']==='actif'?' ولا يزال حتى تاريخه':'' ?>.</p>
            ...
            <?php elseif ($docLang === 'fr'): ?>
                ...
                <?php if (in_array($type,['salaire','travail','cnss'])): ?>
                <p><strong><?= e($nomFr) ?></strong>, exerçant la fonction de <strong><?= e($fnSubj['fr']) ?></strong><?php if ($emp['hire_date']): ?>, engagé(e) depuis le <strong><?= $hireFmt ?></strong><?php endif; ?><?php if ($isTit && $emp['titularization_date']): ?>, titularisé(e) le <strong><?= $titFmt ?></strong><?php endif; ?>, fait partie de notre personnel<?= $emp['status']==='actif'?' et est actuellement en activité':'' ?>.</p>
            ...
            <?php else: /* en */ ?>
                ...
                <?php if (in_array($type,['salaire','travail','cnss'])): ?>
                <p><strong><?= e($nomFr) ?></strong>, holding the position of <strong><?= e($fnSubj['en']) ?></strong><?php if ($emp['hire_date']): ?>, employed since <strong><?= $hireFmt ?></strong><?php endif; ?><?php if ($isTit && $emp['titularization_date']): ?>, tenured on <strong><?= $titFmt ?></strong><?php endif; ?>, is a member of our staff<?= $emp['status']==='actif'?' and is currently in service':'' ?>.</p>
...
<?php
    ...
    $__doc = preg_replace('~(مدرسة|مؤسسة|دير|مركز|دار|مستوصف|ثانوية)(\s*:?\s*)<strong>\1\s+~u', '$1$2<strong>', $__doc);
    $__doc = preg_replace('~((?:l[\x27\x{2019}])?[EeÉé]cole)(\s*:?\s*)<strong>(?:[EeÉé]cole)\s+~u', '$1$2<strong>', $__doc);
    $__doc = preg_replace('~((?:the\s+)?school)(\s*:?\s*)<strong>(?:School|school|[EeÉé]cole)\s+~u', '$1$2<strong>', $__doc); // بالإنكليزي الاسم الفرنسي «Ecole …» بعد «school» تكرار للمعنى نفسه
    if ($idLine !== '') {
        if (preg_match_all('~<div style="(width:2[68]0px;margin:42px|display:flex;justify-content:space-between;margin-top:[2-5][0-9]px)~', $__doc, $__mm, PREG_OFFSET_CAPTURE)) {
            $__pos = end($__mm[0])[1];
            $__doc = substr($__doc, 0, $__pos) . $idLine . substr($__doc, $__pos);
        } else {
            $__p1 = strrpos($__doc, '</div>'); $__p2 = strrpos(substr($__doc, 0, (int)$__p1), '</div>');
            if ($__p2 !== false) $__doc = substr($__doc, 0, $__p2) . $idLine . substr($__doc, $__p2);
        }
    }
    echo $__doc;
?>
